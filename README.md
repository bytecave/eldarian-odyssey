...
Windows builds are validated with the PureBasic 6.41 x64 ASM compiler (`pbcompiler.exe`). From the repository directory, run `powershell -NoProfile -File .\Validate.ps1` to check the source, build release/debug executables, and run the parser/state regression suite. The script accepts `-Compiler` for another installation path. Use the ASM backend because the grayscale module contains inline assembly. The script embeds the Windows 1.03 version resource in both x64 executables. Generated executables stay in `Binaries` and are ignored by Git; the two published x64 ZIP archives are tracked there.

The historical x86 project targets are retained, but x86 and non-Windows builds have not been validated in this migration. See [the September 2026 review and manual regression guide](REVIEW-AND-WALKTHROUGH-2026-09.md) for results, remaining limitations, and a complete test walkthrough. **That guide and the regression test source contain major game spoilers.**
